// 角色属性页（占位实现）。页面模块契约见 renderer/pages/README.md。
// 本文件由「角色属性」功能开发者独占：只改这里与 pages/heroes.css。
(function () {
  "use strict";

  var PAGE_KEY = "heroes";
  var DATA_NAME = "heroes"; // ctx.getGameData("heroes") → resources/heroes.json
  ...
  function template(helpers) {
    return "" +
      ...
      "<div><h2>功能开发中</h2><p>页面脚手架已就绪，业务逻辑请在 renderer/pages/heroes.js 内实现</p></div>" +
      "</div>" +
      "<div class='page-status-row'>" +
      helpers.pill("页面 " + PAGE_KEY, "purple") +
      "<span class='page-data-status' data-testid='heroes-data-status'>正在检查数据文件…</span>" +
      "</div>" +
  ...
  function refresh(ctx) {
    if (!dom) return;
    var status = dom.querySelector("[data-testid='heroes-data-status']");
    if (!status) return;
  ...
  window.NightreignPages[PAGE_KEY] = {
    // 首次切换到本页时调用一次；mount 是 [data-mount="heroes"] 元素。
