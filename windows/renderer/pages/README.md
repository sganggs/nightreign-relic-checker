...
三个新页面 —— `bosses`（首领数据）、`lookup`（词条反查）、`ranker`（增伤排名）——
各自只由一对文件实现：
...
`core.js`、`styles.css`、`main.go`、`bindings.go` 都不需要再动，三个页面之间也
不会产生冲突。
...
- `index.html` 已经按顺序引入 `pages/bosses.js`、`pages/lookup.js`、
  `pages/ranker.js`（都在 `app.js` 之前，`defer`），以及三个 `pages/<key>.css`。
