...
// 占位页：由「增伤排名」车道整文件替换，签名保持不变（见 android/PAGES.md）。
...
internal fun RankerScreen(
    ...
    val skills = rememberGameDataHeader(GameDataKey.SKILLS)
    val buffs = rememberGameDataHeader(GameDataKey.BUFFS)
    GameDataScreenScaffold(
        title = DataPage.RANKER.title,
        subtitle = DataPage.RANKER.eyebrow,
        onBack = onBack,
        state = skills.zip(buffs),
        modifier = modifier,
        statusPills = { NightPill("参数表 1.03.5", NightColors.TextMuted) },
    ) { (skillsHeader, buffsHeader) ->
        GameDataPendingNotice(
            detail = DataPage.RANKER.summary + "桌面端 v0.3.0 已提供，手机版正在移植。",
            datasets = listOf(GameDataKey.SKILLS to skillsHeader, GameDataKey.BUFFS to buffsHeader),
        )
    }
}
