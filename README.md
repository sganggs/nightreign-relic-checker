...
- **叠层要手填层数**（填了层数即视为条件成立）：遗物词条「每次打倒封印监牢里的囚犯，能提升攻击力」一局实际最多 7 层（地图参数里每张地图最多 7 座封印监牢，与玩家反馈一致；参数表共 10 层）、「每次打倒黑夜入侵者，能提升攻击力」按 4 层（参数表里查不到每局上限，取玩家反馈），这两条在遗物卡上填，超出实际上限会提示；「赐福王的余威」在其它增益 · 局内叠层里按本局新发现的赐福数手填（每份 ×1.02，参数表没有上限）。
