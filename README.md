...
**分组口径（第三版起，按出场场合）**：分组看每条数值行实际在哪出场（地图 MSB 的放置 + 远征抽选参数判定出的 `roles`，每个场合都附出处「表名 行 · 地图」），**不看威胁档位**——`tier` / `threat` 只是多人缩放档位名，铃珠猎人野外版挂的就是「守夜首领威胁档」，按它分组会把一批场景头目错归到守夜首领，现在只在展开区留一行「威胁档位」小字。默认六组：夜王 / 守夜首领 / 据点首领 / 场景头目 / 封印监牢 / 其它场合（守夜前哨、坑道精英、大空洞高塔首领、突袭事件、黑夜入侵者、地图事件、其他地图都并在这里）。夜王卡只进「夜王」，它们的突袭 / 地图事件 / 未放置场合只作卡头徽标；其余首领一组有几个场合就同时出现在几个分组里，折叠态的代表行跟着当前分组走：先只留场合落在该分组的行，再依次收敛到主战行、排掉登场演出 / 血条实体、排掉不掉奖励的行，最后取血量最高的一条（救世旗手因此显示实战的「二阶段」，不再是未放置的蠕虫行）。「随从/召唤物」「未放置」是两个默认隐藏的分组，与下面说的非首领实体共用「显示隐藏实体」开关：只出现在这两个场合的 8 组连同 4 组非首领实体默认不显示（共 12 组，页面底部逐一列名），展开区里只属于这两个场合的 148 条数值行也默认收起。
