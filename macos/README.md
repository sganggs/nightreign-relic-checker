...
- **增伤排名**（v0.3.0 新增） — 选一个战技（+ 武器）或法术并勾选命中分段、解出伤害构成，再自己组一套局内配置：常规 / 深夜开关（武器词条 6 把 × 1 条 / 6 把 × 2 条正面，深夜专属正面每把最多 1 条；遗物 3 件 / 3 普通 + 3 深夜），局内武器词条、遗物（固定词条遗物整件，或自组三词条并按词条检查口径实时校验、深夜遗物自动配诅咒）、护符两槽、其它增益四栏，汇总按互斥键去重后连乘再按构成加权，给出总倍率、各栏小计与「按推荐填满」。生效判定用数据集预计算的 `appliesTo`，不生效项默认隐藏、可显示并标出原因。
